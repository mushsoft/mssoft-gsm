  ...
  return (
    <div className="group bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden hover:border-slate-700 transition-all flex flex-col justify-between">
      ...
      <div className="relative aspect-video w-full bg-slate-950 p-4 flex items-center justify-center overflow-hidden">
        ...
        {/* Port Type Badge */}
        <span className="absolute top-3 right-3 z-10 px-2 py-0.5 rounded text-[10px] font-mono font-bold bg-slate-800 text-slate-200 border border-slate-700">
          {flex.portType}
        </span>
      ...
      <div className="p-4 flex-1 flex flex-col justify-between space-y-3">
        <div>
          ...
          <h3 className="font-bold text-slate-100 text-sm line-clamp-2">{flex.title}</h3>
        ...
        <div className="space-y-1.5 pt-2 border-t border-slate-800/80 text-xs text-slate-400">
          ...
          <div className="flex justify-between">
            ...
            <span className="text-slate-200 font-medium">{flex.includesMic ? 'Yes (Built-in Mic)' : 'Flex Only'}</span>
          </div>
          <div className="flex justify-between">
            <span>Data Sync / OTG:</span>
            <span className="text-slate-200 font-medium">{flex.supportsOTG ? 'Yes' : 'Charging Only'}</span>
          </div>
        </div>
