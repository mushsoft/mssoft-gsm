...
export default function DeleteCouponButton({ couponId, couponCode }: { couponId: string; couponCode: string }) {
  ...
  async function handleDelete() {
    ...
    try {
      ...
      if (!response.ok || !data.success) {
        setError(data.error || 'Unable to delete coupon');
        setIsDeleting(false);
        setConfirming(false);
        return;
      }
    ...
    return (
      <div className="rounded-lg border border-red-200 dark:border-red-900/60 bg-red-50 dark:bg-red-950/40 p-3">
        <p className="mb-2 text-xs text-red-600 dark:text-red-300">
          Delete &quot;{couponCode}&quot;? This can&apos;t be undone.
        </p>
