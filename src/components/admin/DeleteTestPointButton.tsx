...
export default function DeleteTestPointButton({ testPointId, testPointTitle }: { testPointId: string; testPointTitle: string }) {
  ...
  async function handleDelete() {
    ...
    try {
      const response = await fetch(`/api/admin/testpoints/${testPointId}`, { method: 'DELETE' });
      const data = await response.json();

      if (!response.ok || !data.success) {
        setError(data.error || 'Unable to delete test point');
        setIsDeleting(false);
        setConfirming(false);
        return;
      }

      router.push('/admin/testpoints');
      router.refresh();
    } catch {
    ...
    return (
      <div className="rounded-lg border border-red-200 dark:border-red-900/60 bg-red-50 dark:bg-red-950/40 p-3">
        <p className="mb-2 text-xs text-red-600 dark:text-red-300">
          Delete &quot;{testPointTitle}&quot;? This can&apos;t be undone.
        </p>
